...
/**
 * Thrown when the {@link Tokenizer} cannot resolve a symbol sequence as a
 * known operator.
 * <p>
...
public final class UnknownOperatorException extends MathException {
    ...
    /**
     * Creates a new exception for an unrecognized operator.
     *
     ...
     * @param length the length of the operator that was being parsed
     */
    public UnknownOperatorException(@NotNull String expression, int position, int length) {
        super("Unknown operator '" + token(expression, position, length) + "' at pos " + position + " in expression '" + expression + "'");
